...
class SeedService(
    ...
    fun seedHierarchy(): SeedResult {
        ...
        val categoryCache = mutableMapOf<String, UUID>()
            ...
            for (agentSlot in 1..3) {
                ...
                val agent = agentRepository.save(
                    AgentJpaEntity(
                        ...
                        phone = agentPhone, email = agentEmail, agentCode = agentCode,
                        lbc = lbc, region = lbc.region, district = lbc.district,
                        status = AgentStatus.ACTIVE,
                        latitude = null, longitude = null, lastSeen = null, joinedDate = now
                    )
                )
                agentCount++
                ...
                for (farmerSlot in 1..3) {
                    ...
                    val farmer = farmerRepository.save(
                        FarmerJpaEntity(
                            id = UUID.randomUUID(), farmerCode = farmerCode,
                            firstName = fFirst, lastName = fLast,
                            phone = farmerPhone, email = "farmer${globalFarmerIdx}@soiltechlbc.com",
                            nationalId = "GHA-%08d".format(globalFarmerIdx),
                            agentId = agent.id!!, lbcId = lbc.id!!,
                            region = lbc.region, district = lbc.district, community = community,
                            status = FarmerStatus.APPROVED, kycVerified = true,
                            cropTypesRaw = crop.name, fullName = "$fFirst $fLast",
                            syncStatus = "SYNCED", joinedDate = now
                        )
                    )
                    farmerCount++
                    ...
                    val farm = farmRepository.save(
                        FarmJpaEntity(
                            id = UUID.randomUUID(), farmerId = farmer.id!!,
                            name = "${crop.name} Farm – ${farmer.firstName}",
                            sizeHectares = 1.5 + (globalFarmerIdx % 5) * 0.5,
                            cropType = crop.name, location = "$community, ${lbc.district}",
                    ...
                    for (slot in 1..2) {
                        ...
                        val record = produceRecordRepository.save(
                            ProduceRecordJpaEntity(
                                id = UUID.randomUUID(), farmerId = farmer.id!!, farmId = farm.id,
                                agentId = agent.id!!, cropType = crop.name, cropVariety = crop.variety,
                                grade = crop.grade, quantityKg = qty, pricePerKg = crop.price,
                                totalAmount = total, status = CollectionStatus.COLLECTED,
                                collectedAt = now.minusDays((slot * 7).toLong()),
                                notes = "Seeded – ${lbc.region}", photosRaw = null, syncStatus = SyncStatus.SYNCED
                            )
                        )
                        produceCount++
                        ...
                        val listing = produceListingRepository.save(
                            ProduceListingJpaEntity(
                                id = UUID.randomUUID(), produceRecordId = record.id!!,
                                farmerId = farmer.id!!, farmId = farm.id, agentId = agent.id!!, lbcId = lbc.id!!,
                                cropType = crop.name, cropVariety = crop.variety, grade = crop.grade,
                                totalQuantityKg = qty, availableQuantityKg = qty, pricePerKg = crop.price,
                                status = ProduceListingStatus.AVAILABLE,
                                region = lbc.region, district = lbc.district,
                                agentName = "${agent.firstName} ${agent.lastName}",
                                farmerName = "$fFirst $fLast", lbcName = lbc.name,
                                photosRaw = null, collectedAt = record.collectedAt
                            )
                        )
                        listingCount++
                            ...
                            productCategoryRepository.findByNameIgnoreCase(crop.name)?.id
                                ?: productCategoryRepository.save(
                                    ProductCategoryJpaEntity(
                                        id = UUID.randomUUID(), name = crop.name,
                        ...
                        productRepository.save(
                            ProductJpaEntity(
                                id = UUID.randomUUID(), categoryId = categoryId,
                                produceListingId = listing.id!!, farmerId = farmer.id!!, agentId = agent.id!!,
                                name = "${crop.name} – ${crop.variety}",
                                description = "Fresh ${crop.name} from $fFirst $fLast. ${lbc.district}, ${lbc.region}. Grade: ${crop.grade}",
                                pricePerUnit = crop.price, unit = "kg", stockQuantity = qty.toInt(),
                                isAvailable = true, imageUrl = null, isOnDeal = false,
                                isFeatured = slot == 1 && globalFarmerIdx % 5 == 0,
                                originalPrice = null, farmerName = "$fFirst $fLast",
                                location = "${lbc.district}, ${lbc.region}",
                                freshnessLabel = "Grade ${crop.grade}",
                                averageRating = BigDecimal.ZERO, reviewCount = 0
                            )
                        )
                        productCount++
                    }
        ...
        val result = SeedResult(agentCount, farmerCount, farmCount, produceCount, listingCount, productCount)
        log.info("seedHierarchy done — {}", result)
        return result
    }
}
